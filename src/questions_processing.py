...
class QuestionsProcessor:
    ...
    def _extract_references(self, pages_list: list, retrieval_results: list) -> list:
        # 根据验证后的页码，从检索结果中提取真实 PDF sha1 和页码。
        refs = []
        seen = set()
    ...
    def _print_retrieval_sources(self, retrieval_results: list):
        """打印本次检索命中的原始文件和页码，方便后端排查来源。"""
        print("[检索来源] 原始文件和页码：")
        for index, result in enumerate(retrieval_results, start=1):
            file_name = result.get("file_name", "")
            page = result.get("page", "")
            distance = result.get("distance", "")
            print(f"[检索来源] {index}. 原始文件={file_name}，页码={page}，检索分数={distance}")
    ...
    def get_answer_for_company(self, company_name: str, question: str, schema: str) -> dict:
        ...
        self._print_retrieval_sources(retrieval_results)
        t4 = time.time()
    ...
    def _extract_companies_from_subset(self, question_text: str) -> list[str]:
        ...
        if not found_companies and len(company_names) == 1:
            found_companies.append(company_names[0])
